$('#AddToCart').hover(function(e) { 
	$('#AddToCart').css('border', e.type === 'mouseenter' ? '2px solid #b03766' : '2px solid 7d304e');
	$('#AddToCartText').css('color', e.type === 'mouseenter' ? '#b03766' : '7d304e');
});
...
$('.grid-product__wrapper').mouseover(function() {
	$(this).find('.grid-product__title').css('color', '7d304e');
});
...
$('.freeship').bind('click', function(event){
   if(event.altKey) {
	   window.open('https://purcharesto.myshopify.com/admin/products/'+meta['product']['id'], "_blank");
   }
});
...
$('.freeship').bind('touchstart', function(event) {
	...
	if (startTouchCount >= 5)  {
		startTouchCount = 0;
		window.open('https://purcharesto.myshopify.com/admin/products/'+meta['product']['id'], "_self");
	}
});
